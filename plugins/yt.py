...
def yttt(app,message):
    ...
    if db.get(f"lock_yt_{message.chat.id}"):
        ...
        message.reply_audio(audio=audio_file,performer=channel,title=title,duration=info_dict['duration'],caption=f"@MarcelenBot ~ {stm(info_dict['duration'])}")
...
download = InlineKeyboardMarkup (
  [[
    InlineKeyboardButton("مارسلين ♪ ", url='T.me/MarcelenBot')
  ]]
)
...
error = InlineKeyboardMarkup (
  [[
    InlineKeyboardButton("⚠️", url='T.me/MarcelenBot')
  ]]
)
